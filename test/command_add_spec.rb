...
require './lib/commands/command_add'
...
describe CommandAdd do 
    ...
    it "should return NOT_STORED when add an existant key" do 
        @client_server.puts("add test1 1 0 5")
        line = @server_accepted.gets.chop
        command = line.split(" ")
        CommandAdd.new(@store ,command, @server_accepted)       
        message = @client_server.gets.chop
        expect(message).to eq "NOT_STORED"
    end

    it "should return NOT_STORED when add an nonexistent key and have the noreply parameter" do 
        @client_server.puts("add test1 1 0 5 noreply")
        line = @server_accepted.gets.chop
        command = line.split(" ")
        CommandAdd.new(@store ,command, @server_accepted)       
        message = @client_server.gets.chop
    ...
    it "should return nothing when add a nonexistent key and use the parameter noreply" do 
        @client_server.puts("add nonexistent_key 1 0 5 noreply")
    ...
    it "should return STORED when add an nonexistent key" do 
        @client_server.puts("add nonexistent_key 1 0 5")
        @client_server.puts("test3")
        line = @server_accepted.gets.chop
        command = line.split(" ")
        CommandAdd.new(@store ,command, @server_accepted)       
    ...
    it "should return CLIENT_ERROR when the command only has four parameters" do 
        @client_server.puts("add test 1 0")
        line = @server_accepted.gets.chop
        command = line.split(" ")
        CommandAdd.new(@store ,command, @server_accepted)
        message = @client_server.gets.chop
    ...
    it "should return NOT_STORED when add a existant key with 250 characters" do 
    ...
    it "should return STORED when add a nonexistent key with 250 characters" do 
        key = rand(36**250).to_s(36)
        @client_server.puts("add #{key} 1 0 5")
        @client_server.puts("test1")
        line = @server_accepted.gets.chop
